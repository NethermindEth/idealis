...
def parse_trace_call(
    ...
    return_events += [
        EventResponse(
            ...
            contract_address=to_bytes(event["address"]),
            data=to_bytes(event.get("data")),
            topics=[to_bytes(topic) for topic in event["topics"]],
        )
    ...
    if "revertReason" in trace_call:
        ...
        error = (
            TraceError.reverted
            if error_text and error_text.startswith("Reverted")
            else None
        )
    ...
    if trace_call["type"] in ["CREATE", "CREATE2"]:
        return_create_traces.append(
            CreateTraceResponse(
                ...
                error=error,
                from_address=to_bytes(trace_call["from"]),
                deployed_code=to_bytes(trace_call["output"]),
                created_contract_address=to_bytes(trace_call["to"]),
        ...
        return_call_traces.append(
            CallTraceResponse(
                ...
                from_address=to_bytes(trace_call["from"]),
                to_address=to_bytes(trace_call["to"]),
...
def unpack_trace_block_response(
    ...
    list[CallTraceResponse],
    list[CreateTraceResponse],
    list[RewardTraceResponse],
    list[SuicideTraceResponse],
]:
    """
...
def parse_call_trace(
    ...
    return CallTraceResponse(
        ...
        error_text=None,
        from_address=to_bytes(trace_dict["action"]["from"]),
        to_address=to_bytes(trace_dict["action"]["to"]),
...
def parse_create_trace(trace_dict: dict[str, Any]) -> CreateTraceResponse:
    return CreateTraceResponse(
        ...
        created_contract_address=to_bytes(trace_dict["result"]["address"]),
        gas_supplied=hex_to_int(trace_dict["action"]["gas"]),
